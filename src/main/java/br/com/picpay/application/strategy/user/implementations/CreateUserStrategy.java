...
public class CreateUserStrategy implements ICreateUserStrategy {
    private final UserApplicationService userApplicationService;
    private final BalanceApplicationService userBalanceApplicationService;
    private final WalletApplicationService walletApplicationService;

    @Override
    public UserResponse createUser(UserRequest userRequest) {
        User userEntity = this.userApplicationService.createUser(userRequest);

        BalanceResponse balanceResponse = this.userBalanceApplicationService.createBalanceForUser(userRequest.getBalance(), userEntity);

        Wallet walletRequest = Wallet.builder()
                .balance(balanceResponse.getBalanceId())
                .type(WalletType.USER_BALANCE)
                .build();
